...
func itemSchemaExample() {
	...
	stringItemSchema := schema.NewString().
		MinLength(3).
		Pattern("^[a-zA-Z]+$").
		Build()
...
func containsExample() {
	...
	containsSchema := schema.NewNumber().Min(10).Build()
	arraySchema := schema.NewArray().
...
func nestedArraysExample() {
	...
	innerArraySchema := schema.NewArray().
		Items(schema.NewString().Build()).
		Range(1, 3). // Each inner array has 1-3 strings
		Build()

	outerArraySchema := schema.NewArray().
...
func commonPatternsExample() {
	...
	// String list
	stringListSchema := schema.NewArray().
		StringArray().
		NonEmpty().
		Build()

	// Set of unique IDs
	idSetSchema := schema.NewArray().
		Items(schema.NewInteger().ID().Build()).
		Set(). // Unique items
		Build()

	// Fixed-size tuple
	tupleSchema := schema.NewArray().
		Tuple(3).
		Description("3-element tuple").
		Build()

	// Limited list
	limitedListSchema := schema.NewArray().
		LimitedList(10).
