...
func (b *ServiceBuilder) SimpleMethod(name string, inputSchema api.Schema, outputSchema api.Schema) *ServiceBuilder {
	functionSchema := NewFunctionBuilder().
...
func (b *ServiceBuilder) CRUDService(entityName string) *ServiceBuilder {
	...
	b.SimpleMethod("Delete",
		NewString().Build(),  // Input: ID
		NewBoolean().Build()) // Output: success

	// List
	b.SimpleMethod("List",
		NewObject().Build(), // Input: query parameters
		NewArray().Build())  // Output: array of entities
...
func (b *ServiceBuilder) EventService() *ServiceBuilder {
	...
	b.SimpleMethod("Subscribe",
		NewString().Build(),  // Input: event type
		NewBoolean().Build()) // Output: success
