...
// ToJSONSchema generates a JSON Schema representation of the object schema.
func (o *ObjectSchema) ToJSONSchema() map[string]any {
	jsonSchema := map[string]any{
		"type": "object",
	}

	// Add properties
	if len(o.config.Properties) > 0 {
		properties := make(map[string]any)
		for propName, propSchema := range o.config.Properties {
			properties[propName] = propSchema.ToJSONSchema()
		}
		jsonSchema["properties"] = properties
	}

	// Add required properties
	if len(o.config.Required) > 0 {
		jsonSchema["required"] = o.config.Required
	}

	// Add additional properties
	jsonSchema["additionalProperties"] = o.config.AdditionalProperties

	// Add property constraints
	if o.config.MinProperties != nil {
		jsonSchema["minProperties"] = *o.config.MinProperties
	}

	if o.config.MaxProperties != nil {
		jsonSchema["maxProperties"] = *o.config.MaxProperties
	}

	// Add pattern properties
	if len(o.config.PatternProperties) > 0 {
		patternProps := make(map[string]any)
		for pattern, schema := range o.config.PatternProperties {
			patternProps[pattern] = schema.ToJSONSchema()
		}
		jsonSchema["patternProperties"] = patternProps
	}

	// Add dependencies
	if len(o.config.PropertyDependencies) > 0 {
		jsonSchema["dependencies"] = o.config.PropertyDependencies
	}

	// Add metadata
	if o.config.Metadata.Description != "" {
		jsonSchema["description"] = o.config.Metadata.Description
	}

	if len(o.config.Metadata.Examples) > 0 {
		if len(o.config.Metadata.Examples) == 1 {
			jsonSchema["example"] = o.config.Metadata.Examples[0]
		} else {
			jsonSchema["examples"] = o.config.Metadata.Examples
		}
	}

	if len(o.config.Metadata.Tags) > 0 {
		jsonSchema["tags"] = o.config.Metadata.Tags
	}

	if o.config.DefaultVal != nil {
		jsonSchema["default"] = o.config.DefaultVal
	}
